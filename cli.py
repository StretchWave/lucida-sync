...
def cli():
    """Lucida-Sync - Download music from various streaming services via Lucida.to"""
    pass
...
def download(url, output):
    ...
    # client is initialized here, class imported globally
    client = LucidaClient()
    
    click.echo(f"Initializing download for: {url}")
    result = client.download_track(url, output_path=output)
    if result.get("success"):
        click.secho(f"✓ Download complete: {result['filepath']}", fg="green")
